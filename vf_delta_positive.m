...
for j = 2:Ny(1)-1
    vf(1,j) = -H0/(dx*dy)*(c(1,j)*Q(1,j)-c0*Q(1,j) ...
        + c(1,j)*F_tb(1,j) - c(1,j-1)*F_tb(1,j-1) ...
        - H0/Pe*((c(1,j+1)-2*c(1,j)+c(1,j-1))*dx/dy ...
        + (c(2,j)-c(1,j))*dy/dx));
end

% Rectangular volume in the bottom left corner
vf(1,1) = -H0/(dx*dy)*(c(1,1)*Q(1,1)-c0*Q(1,1) + c(1,1)*F_tb(1,1) ...
    - H0/Pe*((c(1,2)-c(1,1))*dx/dy + (c(2,1)-c(1,1))*dy/dx));

% Top triangles
for i = 1:Nx
    vf(i,Ny(i)) = -2*H0/(dx*dy)*(c(i,Ny(i))*Q(i,Ny(i)) ...
        - c(i,Ny(i)-1)*F_tb(i,Ny(i)-1) ...
        - H0/Pe*((-c(i,Ny(i)) + c(i,Ny(i)-1))*dx/dy ...
        + (c(i+1,Ny(i))-c(i,Ny(i)))*dy/dx));
end
